defmodule SNMPMgr.PerformanceTest do
  ...
  defp convert_to_simple_erlang_pdu(pdu) do
    ...
    error_status = :noError  # Use atom instead of numeric
    ...
    erlang_varbinds = Enum.with_index(varbinds, 0) |> Enum.map(fn {{oid, type, value}, index} ->
        ...
        try do
          ...
          case :snmp_pdus.enc_pdu(erlang_pdu) do
            encoded when is_list(encoded) ->
              ...
              IO.puts("  First 20 bytes: #{inspect(binary_part(encoded_binary, 0, min(20, byte_size(encoded_binary))))}")
            {:error, reason} ->
              IO.puts("  Encoding failed: #{inspect(reason)}")
            other ->
